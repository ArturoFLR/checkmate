...
function Slideshow({ player }: SlideshowProps) {
	...
	if (player === "p2") {
		// If player 2 (p2) is selecting his / her portrait, the portrait already selected by player 1 (playersData[0].portrait) will be removed from the array of available portraits for player 2.
		const noSelectedPortraitsPathsCopy = [...noSelectedPortraitsPaths];

		const pathToDeleteIndex = noSelectedPortraitsPathsCopy.findIndex(
			(element) => {
	...
	if (actualPortraitIndex > noSelectedPortraitsPaths.length - 1) {
		// 		If player 1 chose the last portrait from the "noSelectedPortraitsPaths" array, "currentPortraitIndex" will be left with a value greater than the length of the array, causing errors for player 2. This code corrects this situation.
	...
	if (player === "p1") {
		// Every time a portrait is chosen (the one that appears by default also counts) it is set as the one chosen by player 1 or 2 in the PlayersDataContext context
	...
	if (actualPortraitIndex < noSelectedPortraitsPaths.length - 1) {
		// 	Defines the initial portrait that contains the nextPortraitPath variable, based on the portrait that contains the currentPortraitIndex variable
	...
	if (actualPortraitIndex > 0) {
		// 	Defines the initial portrait that contains the prevPortraitPath variable, based on the portrait that contains the currentPortraitIndex variable
		prevPortraitPath = noSelectedPortraitsPaths[actualPortraitIndex - 1];
	} else {
		prevPortraitPath =
			noSelectedPortraitsPaths[noSelectedPortraitsPaths.length - 1];
	}

	function handleNextPortrait() {
		const nextImageContainer = document.getElementById(
			"nextImageContainer"
		) as HTMLDivElement;
	...
	function handlePrevPortrait() {
		const prevImageContainer = document.getElementById(
			"prevImageContainer"
		) as HTMLDivElement;
