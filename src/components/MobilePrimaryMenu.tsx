...
import styles from "./MobilePrimaryMenu.module.scss";
...
function MobilePrimaryMenu() {
	...
	const {mobileMenuState, setMobileMenuState} = useMobileMenuStateContext();		// A context has been used so that the "MainMenuMobile" component can reset the state to "init" when the "X" is pressed to close the menu, and so that no sub-options are left open
	...
	const loadData1 = localStorage.getItem("checkMateSave1Mobile");					// Here the data of each game is saved in JSON format.
	const loadData2 = localStorage.getItem("checkMateSave2Mobile");
	const loadData3 = localStorage.getItem("checkMateSave3Mobile");
	...
	function handleNewGameClick () {
		setMobileMenuState("new");
	}

	function handleLoadGameClick () {
		if (mobileMenuState !== "load") {
			setMobileMenuState("load");
		} else {
			setMobileMenuState("init");
		}
	}

	function handlePlayerVsComputerClick () {
		const mobileMenu = document.getElementById("mobileMainMenuContainer") as HTMLDivElement;				// Used to make the mobile menu disappear, which is in the middle of the screen.
