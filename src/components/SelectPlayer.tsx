...
function SelectPlayer() {
	const [playerDataToShow, setPlayerDataToShow] =
		useState<playerDataToShow>("player1"); // This state changes the player data form displayed, player 1 or player 2
	...
	const aiDefaultLevel = 6;
	let levelSelected = aiDefaultLevel;

	let inputNameErrorTimeout: number;

	function handleP1EnterKeyPress(
	...
	function handleP2EnterKeyPress(
		event: React.KeyboardEvent<HTMLInputElement>
	): void {
		if (event.key === "Enter") {
	...
	// Activates an animation in the input field to draw the user's attention when the text entered is invalid.
	...
	function handleDifficultyClick(
		...
		const containerToHighlight = (event.target as HTMLParagraphElement)
	...
	function removeHighlightedDifficultStyles() {
		const removeHighlightedDifficultStyles = document.querySelector(
			`.${styles.difficultySelected}`
		);
		if (removeHighlightedDifficultStyles)
			removeHighlightedDifficultStyles.classList.remove(
				styles.difficultySelected
			);
	}
	...
	if (playerDataToShow === "player1" && isAIGameData.isAIGame) {
		return (
			<div className={styles.mainContainer}>
				<div
					className={`${styles.playerInfoContainer} ${styles.aiGamePlayerInfoContainer}`}
				>
					...
					<div className={styles.difficultLevelsContainer}>
						<div className={styles.difficultyContainer}>
							<p id="1" onClick={handleDifficultyClick}>
								Fácil
							</p>
						</div>

						<div
							className={`${styles.difficultyContainer} ${styles.difficultySelected}`}
						>
							<p id="6" onClick={handleDifficultyClick}>
								Normal
							</p>
						</div>

						<div className={styles.difficultyContainer}>
							<p id="13" onClick={handleDifficultyClick}>
								Difícil
							</p>
						</div>
					</div>
					...
					<input
						className={styles.playerInfoInput}
						maxLength={9}
						id="playerInfoInput"
						placeholder="Tu nombre"
						onKeyDown={handleP1EnterKeyPress}
						autoComplete="off"
					></input>
					<button
						type="button"
						className={styles.playerInfobtnOk}
						onClick={handlePlayer1OkClick}
					>
						Ok!
					</button>
				</div>
			...
			<div className={styles.mainContainer}>
				<div className={styles.playerInfoContainer}>
					...
					<input
						...
						id="playerInfoInput"
						placeholder="Tu nombre"
						onKeyDown={handleP2EnterKeyPress}
						autoComplete="off"
					></input>
					<button
						type="button"
						className={styles.playerInfobtnOk}
						onClick={handlePlayer2OkClick}
					>
						Ok!
					</button>
				</div>
